...
def main():
    ...
    chThread_RtpRecvr = threading.Thread(target=startReceivingMedia, args=(local_ip, 18, ""))
    chThread_RtpRecvr.start()
    if isUac:
        chThread_loadRunner = threading.Thread(target=startTraffic, args=(sipCallId_map,msg_q, sigSocket,num_sessions,caller,callee,domain,isReliable))
        chThread_loadRunner.start()
        chThread_monitorSipCall = threading.Thread(target=monitorSipCall, args=(sipCallId_map,msg_q, num_sessions, wait_time))
    ...
    try:
        ...
        while True:
            ...
            if buff_evt == "100":
                ...
                handleInvite(uaSession, raw_msg)
                ...
                if msg_200.getMethod() == "INVITE":
                 ...
                 #   if bye_count == call_count:
                   #     print("**** Bye count match, breaking")
                 #       break
            elif buff_evt == "ACK":
        ...
        print("****** IN Finally")
        print("****** Total call count ", callResp_cnt,"------ Total Successful Calls: ", callSuccess_cnt,
              "++++++ Total Failed Calls: ", callFailed_cnt,"+++Total cleared calls ", bye_count)
...
def monitorSipCall(sipCallId_map, msg_q, num_sessions, wait_time):
    # while True:
    #     if sipCallId_map:
    #         break
    # bye_count = 0
...
def startTraffic(sipCallId_map, msg_q, sigSocket, num_sessions, caller, callee, domain, isReliable):
    call_pumped = 0
    i = 1000
    port = 60000
...
###3rd march
def startReceivingMedia(myIp, prefaudiocodec, rxKey):
    myport = 60000
    __buffSize = 2048
