from sipconstants import CRLF, MediaType, MediaIPType, MediaDirection
import re
...
class SDPMLine:
    def __init__(self):
        ...
        self.__aLines = []
        self.__cLine = None
        self.__mediaType = MediaType.AUDIO.value
        self.__mediaIPType = MediaIPType.IPV4.value
        self.__mediaIP = ""
        self.__mediaPort = 0
        self.__prefCodec = 0
        self.__isSRTP = False
        self.__srtpKey = ""
        self.__mediadir = MediaDirection.SENDRECV.value
        
    def setMline(self, mline, content):
        self.__mLine = mline
        mline = mline.split()
        try:
    ...
    def setCline(self, cline):
        ...
        if self.__mediaIP.find(":") != -1:
            self.__mediaIPType = MediaIPType.IPV6.value
    
    def appendAline(self, aline):
        attr = aline.split('=')[1].strip()
        if attr in ['sendonly','inactive','recvonly']:
            if attr == 'sendonly':
                self.__mediadir = MediaDirection.SENDONLY.value
            elif attr == 'recvonly':
                self.__mediadir = MediaDirection.RECVONLY.value
            elif attr == 'inactive':
                self.__mediadir = MediaDirection.INACTIVE.value
            else:
                self.__mediadir = MediaDirection.SENDRECV.value
        elif attr.startswith("crypto"):
            key = re.search("inline:(.*)",attr).group(1).split("|", 1)[0]
            self.__srtpKey = key
