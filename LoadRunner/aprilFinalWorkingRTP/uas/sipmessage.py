...
class SipMessage:
    ...
    def getFromTag(self):
        frm_hdr = self.getHeader(SipHeaders.FROM.value)        
        tag = re.search('tag=.*', frm_hdr[0]).group()
        tag = tag.split(';')[0]        
        return tag
        
    def getToTag(self):
        to_hdr = self.getHeader(SipHeaders.TO.value)[0]
        tag = re.search('tag=(.*)', to_hdr).group(1)
        return tag
    ...
    def getRouteSet(self, isDlgOwner) -> list:
        routeset = self.getHeader(SipHeaders.RECORDROUTE.value)
        if not routeset:
            return []
        if isDlgOwner:
            return routeset.reverse()
