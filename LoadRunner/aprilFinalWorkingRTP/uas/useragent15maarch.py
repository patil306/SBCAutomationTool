...
def main():
    ...
    logging.warning("tcp handshake should start here")
    if isUac:
    ...
    try:
        ...
        print("****** IN Finally")
        print("****** Total call count ", callResp_cnt, "------ Total Successful Calls: ", callSuccess_cnt,"++++++ Total Failed Calls: ", callFailed_cnt, "+++++ Total Cleared Count ", bye_count)
        logging.warning("****** Total call count %d", callResp_cnt) 
        logging.warning("------ Total Successful Calls: %d", callSuccess_cnt)
        logging.warning("++++++ Total Failed Calls: %d", callFailed_cnt)
        logging.warning( "+++++ Total Cleared Count %d", bye_count)
...
def monitorSipCall(sipCallId_map, msg_q, num_sessions, wait_time, isTrafficComplete):
    ...
    while True:
        ...
        if(callid):
            ...
            pass
            #print("did not get callid")
        #
        # if bye_count == call_count:
        #   break
    isTrafficComplete = True
    logging.warning("*****Total BYE sent", bye_count)
