...
USERAGENTS = ["A_ext", "B_ext", "C_ext", "A_rw", "B_rw", "C_rw", "A_int", "B_int", "C_int", "A_EXT", "B_EXT", "C_EXT",
              "A_RW", "B_RW", "C_RW", "A_INT", "B_INT", "C_INT", "A_ext", "B_ext", "C_ext", "A_rw", "B_rw", "C_rw",
              "A_int", "B_int", "C_int", "a_ext", "b_ext", "c_ext", "a_rw", "b_rw", "c_rw", "a_int", "b_int", "c_int"]

KEYLIST = ["event", "method", "direction", "dialogid", "isindialog", "hascontent", "hascontentchanged",
           "targetnodenames", "mediadirection", "nextnode", "INS_HDR", "REP_HDR", "REM_HDR", "REP_REQURI",
           "REM_REQURIPARAM", "INS_SDPMLINE", "INS_SDPSESSATTR", "audiocodecs", "EXP_HDR", "EXP_HDRVALUE", "EXP_REQURI",
           "EXP_REQURIPARAM", "SLEEP_TIMER", "transfertarget"]
app = Flask(__name__, static_folder="/home/pgokhe/venv/callflow/js-sequence-diagrams-master",
            template_folder="/home/pgokhe/venv/callflow/js-sequence-diagrams-master")
# node ounters
EXT_CNT = 0
INT_CNT = 0
RW_CNT = 0
...
def FindsLineRequiredforNode(NodeName, callflow_str):
        ...
        for line in callflow_str.splitlines():
                if (line != ""):
                        parts = re.split(":",
                                         line)  # parts[0] is in format A->B , parts[1] is messege like INVITE,180etc
...
def GetInfo(data, line, NodeName):
        ...
        data["event"] = met_list[0]

        if (len(met_list) > 2):
                data["method"] = met_list[1]
        ...
        if (data["direction"] == "send"):
                targetnames, nextnodenames = FindTargetName(MyNodeName=NodeName, myMethod=data["event"],
                                                            current_line=line)
        ...
        if ((start != -1) and (end != -1)):
                ...
                for i in parts:
                        ...
                        if ("isindialog:TRUE".upper() in i):
                                data["isindialog"] = True
                        if ("dialogid:".upper() in i):
                                print(i)
                                k = i
                                s = k.find('\"') + 1
                                e = k.rfind('\"')
...
def MakeDictionaryWithInfo(NodeName, related_lines):
        ...
        if (NodeName[-3:] == "ext" or NodeName[-3:] == "EXT"):
                ...
                Dict["domain"] = GLOBALDOMAIN
                Dict["outboundip"] = getOutboundIp(Dict["type"])
        if (NodeName[-2:] == "rw" or NodeName[-2:] == "RW"):
        ...
        while (1):
                ...
                for i in UAnodes:
                     ...
                     #   mapipPid[mapNodeIP[i]] = pid
                        ...
                        sleepduration += 1
                break
        ReportingMechanismFile.startMonitoring(mapNodeIP, tid, test_desc)
...
def processCallflow(callflow_str):
        ...
        for j in NodeInfo:
                ...
                for x in NodeInfo[j]:
                        ...
                        # print(NodeInfo[j])
                        # print(x)
                        if (x not in ["type", "extn", "domain", "transport", "allowmethods", "isreliable",
                                      "featuretags", "outboundip", "displayname"]):
...
def button(callflow, i, test_description):
        global tid
        global test_desc
        tid = i
