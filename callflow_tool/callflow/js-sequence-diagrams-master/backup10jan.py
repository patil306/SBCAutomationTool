...
def copyJsonToVM(filename, hostname):        #type can be ext, int,rw
        ...
        ftp_client.close()
def getRWVMIP(localcounter_rw):
        if(localcounter_rw == 1):
                return RW_VM_IP
        elif(localcounter_rw == 2):
                return RW2_VM_IP
...
def processNode(NodeName, callflow_str):
        ...
        pass
#########################################################
def executeApps(NodeInfo, mapNodeIP):                           #make two list
        for i in reversed(NodeInfo):
                print("executeApps+++++++++")
                print(i)
                print(mapNodeIP[i])
                client = paramiko.SSHClient()
                client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                client.connect(mapNodeIP[i], username="root", password="SIPera_123")
...
def processCallflow(callflow_str):
        ...
        for j in NodeInfo:
                #Len = j.len()
                for x in NodeInfo[j]:
                        if(x not in ["type", "extn", "domain", "transport"]):
        ...
        for i in clients:
                if("_ext" in i):  #write to to ext.json
                        ...
                        if(INT_CNT == 1):
                                ...
                                mapNodeVm[i] = INT_VM_IP
                        else:
                                with open('int2.json', 'w') as f:
                                        json.dump({i: NodeInfo[i]}, f, indent=4)
                                mapNodeVm[i] = INT2_VM_IP
                        ...
                        with open(filname, 'w') as f:
                             json.dump({i:NodeInfo[i]}, f, indent=4)
                             remotevm_ip = getRWVMIP(localcounter_rw)
                        copyJsonToVM(filname, remotevm_ip)
                        mapNodeVm[i] = RW_VM_IP
       ...
       # executeApps(NodeInfo, mapNodeVm)
        print("NODE INFO is")
